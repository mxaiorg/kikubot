...
func MessageTool() ToolDefinition {
	return ToolDefinition{
		...
		StaticSystem: "- Coworkers cannot see anything you write as assistant text or reasoning. " +
			...
			"- To attach a file that already exists on disk — anything you fetched with `download_file` (use `extract_text:false` to keep the raw file) or materialised with `save_attachment`, wrote via `bash_exec`, or a large tool result that was spilled to a path (you'll see a `[The FULL untruncated result … was saved to disk at: <path>]` note in the result) — pass its local path as the attachment's `path` field. The bytes are read from disk, so binary files (images, PDFs, archives) attach intact and large data is never re-emitted through your output. NEVER base64-encode a downloaded file and paste it into `content`: large blobs get truncated mid-stream and produce invalid base64. Reserve `content` for small files you author inline this turn (short reports, exports, plain text).\n" +
			...
			"- `In-Reply-To` and `X-Forwarded` compose: set both to reply in-thread AND deliver the forwarded email's attachments in the same message. The reply's subject and threading are kept; the forwarded files are appended.\n" +
